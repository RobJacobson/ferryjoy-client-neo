...
4. **`getVesselTripPredictionsFromTripUpdate`** — loads prediction model parameters when **`getPredictionModelParametersFromTripUpdate`** is non-null (**`loadPredictionModelParameters`**), enriches the active trip, returns **`vesselTripPredictions`** proposal rows + **`predictedTripTimelineHandoffs`** for the same update.
5. **`updateTimeline`** — domain projection from **`{ pingStartedAt, tripUpdate, predictedTripTimelineHandoffs }`**; derives **`PersistedTripTimelineHandoff`** internally (**`timelineHandoffFromTripUpdate`**).
...
Primary path: **`updateTimeline`** consumes **`RunUpdateVesselTimelineFromAssemblyInput`** (`pingStartedAt`, **`tripUpdate`**, **`predictedTripTimelineHandoffs`**) in `actions/ping/runOrchestratorPing.ts` *before* aggregate persistence. The domain derives **`PersistedTripTimelineHandoff`** from **`VesselTripUpdate`** (**`timelineHandoffFromTripUpdate`**), merges **`PredictedTripTimelineHandoff`** branches in memory, and projects **`actualEvents`** / **`predictedEvents`**; timeline assembly does not read `vesselTripPredictions` from the database on this path. Older O5 handoff: [handoff](../../../docs/handoffs/vessel-orchestrator-o5-timeline-and-cleanup-handoff-2026-04-18.md).
...
- **getVesselTripPredictionsFromTripUpdate** — loads prediction model parameters when needed (**`loadPredictionModelParameters`**), enriches the active trip, returns prediction proposals + **`predictedTripTimelineHandoffs`** per changed vessel.
- **updateTimeline** — **`updateTimeline`** from **`domain/vesselOrchestration/updateTimeline`** runs in `actions/ping/runOrchestratorPing.ts` with **`tripUpdate`** + same-ping **`predictedTripTimelineHandoffs`**; timeline rows are applied in the same per-vessel mutation as trip/prediction writes.
...
WSF API
  ...
  -> getVesselTripPredictionsFromTripUpdate (action query + domain inference + proposals + predictedTripTimelineHandoffs)
