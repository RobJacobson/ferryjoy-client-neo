...
4. **`buildPredictionStagePlan`** + **`loadPredictionContext`** + **`updateVesselPredictions`** — derive Stage 4 inputs once, preload production models when needed, then ML overlay + **`vesselTripPredictions`** proposal rows + **`mlTimelineOverlays`** for the same ping.
...
- **buildPredictionStagePlan** + **loadPredictionContext** + **updateVesselPredictions** — **`buildPredictionStagePlan`** derives active/completed prediction inputs and the optional model preload request once; **`loadPredictionContext`** issues the model-parameter query when that request exists; **`updateVesselPredictions`** computes prediction proposals + ML overlays per changed vessel.
...
WSF API
  ...
  -> buildPredictionStagePlan + loadPredictionContext + updateVesselPredictions (action query + domain ML + proposals + mlTimelineOverlays)
...
- `domain/vesselOrchestration/updateVesselPredictions/` — **`buildPredictionStagePlan`**, **`updateVesselPredictions`**, compatibility helpers **`predictionInputsFromTripUpdate`** / **`predictionModelLoadRequestForTripUpdate`**.
...
`updateVesselOrchestrator` uses one WSF batch per ping, then **`runOrchestratorPing`** in `actions/ping/runOrchestratorPing.ts`: locations are written through standalone `bulkUpsertVesselLocations`, and each changed vessel runs **`updateVesselTrip`** → **`updateLeaveDockEventPatch`** → **`buildPredictionStagePlan`** → **`loadPredictionContext`** → **`updateVesselPredictions`** → **`updateTimeline`** → **`persistVesselUpdates`**.
