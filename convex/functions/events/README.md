...
- `common/` is only for neutral validators shared by table schemas
...
3. `replaceScheduledDockEventsForSailingDay` persists only the scheduled slice, then `reloadActualDockEventsForSailingDay` hydrates actual-domain boundary context from numeric schedule and history rows, loads trip indexes, collects `vesselLocations` (full small snapshot), and upserts actual rows via `upsertActualDockRows`.
...
1. The orchestrator ping pipeline upserts vessel locations, then proposes sparse actual writes via the actual domain and batched predictions via the predicted domain.
...
- Domain scheduled seed row: `DockBoundaryEventRecord`.
- Actual reload history row: `EventReloadHistoryRecord`.
