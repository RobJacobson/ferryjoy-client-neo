...
Thin Convex entrypoints for vessel trips (`queries`, `mutations`, `schemas`). Lifecycle logic lives in `convex/domain/vesselOrchestration/updateVesselTrips/` (import via `domain/vesselOrchestration/updateVesselTrips`), while the orchestrator wires targeted schedule-continuity access and prediction staging from [`updateVesselOrchestrator`](../vesselOrchestrator/action/actions.ts).
