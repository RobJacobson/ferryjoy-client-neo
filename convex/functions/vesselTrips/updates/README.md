...
| `buildTrip.ts` | `buildTrip` — orchestrates all build functions (location, schedule, predictions) with event detection |
...
| `utils.ts` | `tripsAreEqual`, `deepEqual` — equality checking utilities |
...
2. Start new trip via `buildTrip(ctx, currLocation, existingTrip, true)` with `tripStart=true` for `Prev*` context: internally calls `baseTripFromLocation` → `appendInitialSchedule` (if at dock with missing ArrivingTerminal) → `appendFinalSchedule` (if key changed) → `appendArriveDockPredictions` (if just arrived at dock) → `appendLeaveDockPredictions` (if LeftDock defined).
...
1. Build `proposedTrip` via `buildTrip` (which internally calls `baseTripFromLocation` + `appendInitialSchedule` + `appendFinalSchedule` + `appendArriveDockPredictions` + `appendLeaveDockPredictions`).
