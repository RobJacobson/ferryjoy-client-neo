...
- **`vesselOrchestration/`** — **`updateVesselTrips/`** (tick plan `computeVesselTripTickWritePlan` + functions `runProcessVesselTripsTick`, **continuity**, read merges, `vesselTripsBuildTripAdapters`, **`tripLifecycle/`**); `updateTimeline/`; `updateVesselPredictions/`; eligibility; docs. Post-fetch tick orchestration lives in `functions/vesselOrchestrator` (`executeVesselOrchestratorTick`). Map: [`vesselOrchestration/architecture.md`](vesselOrchestration/architecture.md).
