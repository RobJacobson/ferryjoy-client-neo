...
- **`vesselTrips/`** — Vessel active-trip lifecycle for orchestrator ticks: `processVesselTripsWithDeps`, `buildTrip`, projection assembly, and related helpers. **`vesselTrips/continuity/`** holds schedule-backed docked continuity (`resolveDockedScheduledSegment`, `resolveEffectiveDockedLocation`). `convex/adapters/vesselTrips/processTick.ts` wires default `buildTripAdapters` (`resolveEffectiveLocation`, `appendFinalSchedule`) for production callers.
