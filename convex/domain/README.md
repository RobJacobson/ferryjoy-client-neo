...
- **`vesselOrchestration/`** — `runVesselOrchestratorTick`; **`updateVesselTrips/`** (tick entry `processVesselTripsWithDeps`, **continuity**, read merges, `vesselTripsBuildTripAdapters`, **`tripLifecycle/`**); `updateTimeline/`; `updateVesselPredictions/`; eligibility; docs. Map: [`vesselOrchestration/architecture.md`](vesselOrchestration/architecture.md).
