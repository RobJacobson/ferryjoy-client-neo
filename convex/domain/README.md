...
- **`vesselTrips/`** — Vessel active-trip lifecycle for orchestrator ticks: `processVesselTripsWithDeps`, `buildTrip`, projection assembly, and related helpers. The functions layer wires default `buildTripAdapters` (`resolveEffectiveLocation`, `appendFinalSchedule`).
