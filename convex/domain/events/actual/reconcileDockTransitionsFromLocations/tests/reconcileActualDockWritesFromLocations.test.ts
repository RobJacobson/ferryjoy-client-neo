...
describe("buildLocationReconcileBoundaryEvents", () => {
  it("attaches exact schedule-aligned actual rows to scheduled boundaries", () => {
    ...
    const scheduledEvents = buildScheduledDockEvents(events, 0);
    const reconciled = buildLocationReconcileBoundaryEvents({
      scheduledEvents,
      actualEvents: [
        makeActualEvent({
  ...
  it("does not attach physical-only arrival actual rows to scheduled boundaries", () => {
    ...
    expect(reconciled[1]?.EventActualTime).toBeUndefined();
  });
});

describe("reconcileActualDockWritesFromLocations", () => {
  it("emits a departure actual-boundary patch when underway location proves departure", () => {
    ...
    const effects = reconcileActualDockWritesFromLocations({
...
const makeSeedEvents = (segments: SeedSegment[]): DockBoundaryEventRecord[] =>
