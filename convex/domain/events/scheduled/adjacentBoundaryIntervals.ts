/**
 * Builds adjacent dock and sea intervals from ordered boundary events.
 *
 * Scheduled-event consumers use these intervals to resolve trip continuity from
 * explicit boundary adjacency instead of wall-clock proximity.
 */

export type AdjacentBoundaryEvent = {
  Key: string;
  SegmentKey: string;
...
export type AdjacentBoundaryInterval =
 ...
 *
 * @param events - Ordered boundary events for one vessel/day scope
 * @returns Adjacent structural intervals in query/render order
 */
export const buildAdjacentBoundaryIntervals = (
  events: AdjacentBoundaryEvent[]
): AdjacentBoundaryInterval[] => {
  const intervals: AdjacentBoundaryInterval[] = [];
