...
describe("getNextScheduledTripFromExistingTrip", () => {
  it("returns the next scheduled segment when the terminal matches", async () => {
    const nextSegment = makeScheduledSegment({
      Key: "CHE--2026-03-13--12:30--CLI-MUK",
    });

    const match = await getNextScheduledTripFromExistingTrip({
  ...
  it("returns null when the next segment points at a different terminal", async () => {
    const nextSegment = makeScheduledSegment({
