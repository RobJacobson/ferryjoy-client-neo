/**
 * Merges resolved schedule fields onto active-trip rows.
 *
 * Resolvers produce schedule-field snapshots. This module applies those
 * snapshots to a concrete ConvexVesselTrip while preserving safe next-leg
 * continuity rules.
 */
...
type MergeResolvedScheduleFieldsInput = {
...
const mergeResolvedScheduleFields = ({
  ...
  scheduleKeyChanged,
  resolution,
}: MergeResolvedScheduleFieldsInput): ConvexVesselTrip => {
 ...
 *
 * Stale NextScheduleKey and NextScheduledDeparture from a prior segment can
 * mislead downstream logic if the trip key changed or the row lost its schedule
 * attachment; this step removes those hints before attachNextScheduledTripFields
 ...
 * @returns Either the trip unchanged, or a copy with next-leg fields explicitly undefined
 */
 ...
 * @returns True when next-leg fields should be stripped before the next-leg attachment step
 */
 ...
 * ScheduleKey still matches the prior row's key, missing Next fields on the new row
 ...
 * @returns Base trip with NextScheduleKey and NextScheduledDeparture set or cleared
 */
