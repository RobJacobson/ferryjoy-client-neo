...
Prediction inputs are derived once by **`buildPredictionStagePlan`** from
**`VesselTripUpdate`**. Timeline handoff is derived inside **`updateTimeline`** from the same shape
