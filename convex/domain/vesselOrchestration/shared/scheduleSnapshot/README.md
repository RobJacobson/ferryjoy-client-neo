...
- `createScheduleContinuityAccessFromSnapshot` — an implementation of `ScheduleContinuityAccess` backed by that in-memory fixture so `updateVesselTrip` / `tripFields` tests can run without Convex `eventsScheduled` queries.
