...
Production uses memoized targeted `eventsScheduled` reads via `functions/vesselOrchestrator/scheduleContinuityAccess.ts`, which implements the same `ScheduleContinuityAccess` interface.
