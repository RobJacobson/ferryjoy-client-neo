...
/**
 * Replacement row for predicted event writes after
 ...
 */
const completedBoundaryActiveTripWithMlForEvents = (
  fact: CompletedArrivalHandoff
): ConvexVesselTripWithML => {
  if (fact.activeVesselTripWithMl === undefined) {
    throw new Error(
      "CompletedArrivalHandoff.activeVesselTripWithMl is required before event projection; merge predicted trips from getVesselTripPredictionsFromTripUpdate first."
    );
  }
 ...
 * @returns Actual and predicted dock writes for event mutations
 */
...
/**
 * Converts one completed-trip fact into actual and predicted event writes.
 *
...
const pingEventWritesFromCompletedFact = (
  ...
  predictedDockWriteBatches: [
    ...
    buildPredictedDockWriteBatch(
      completedBoundaryActiveTripWithMlForEvents(fact)
    ),
