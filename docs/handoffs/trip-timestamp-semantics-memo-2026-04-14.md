...
- UI copy (field names in code and API are aligned with this memo).
...
**`LeftDockActual`** is the stored **physical departure** boundary (leave origin dock / `dep-dock`). The feed may also expose a raw **`LeftDock`** timestamp on the row; projection and ML prefer **`LeftDockActual`** when the pipeline has asserted departure. Unlike arrival-side naming, departure was **not** overloaded in the same way as `AtDockActual`, but it still sits in the **same taxonomy** as arrival actuals.
...
Use **exactly one field name per concept** (past tense on arrivals). No parallel “canonical + alias” names.

| Field | Role |
|-------|------|
| **`ArrivedCurrActual`** | Asserted arrival at the **origin dock** for **this** sailing, when the pipeline can stand behind it. |
| **`ArrivedNextActual`** | Asserted arrival at the **destination dock** for **this** sailing, when known. |
| **`LeftDockActual`** | Asserted **departure from the departing terminal** (leave origin dock) for **this** sailing — the physical counterpart to `dep-dock` projection and ML. |
...
Both sides are feed timestamps for the **same** boundary instant.
...
On **cold start**, **`StartTime`** is the **first time we recorded** the trip; we do not pretend we know **`ArrivedCurrActual`**. **`EndTime`** may still be set when the row closes even if **`ArrivedNextActual`** is unknown (synthetic close).

**Happy path:** **`StartTime` / `EndTime`** align with the same feed times as the **physical** actuals when full information exists in one coherent story (see **Worked example** below).
...
### Worked example: happy path (one sailing)

All times are **feed** `TimeStamp` values on the ticks that assert each fact.

1. **Trip *N* opens** after **complete-and-start** from trip *N−1* on tick **`t₀`**. On that **same** tick, coverage opens and we assert arrival at the **origin dock** for this leg: **`StartTime = t₀`**, **`ArrivedCurrActual = t₀`**. (This is where legacy **`TripStart`** often **equals** **`StartTime`**: same instant, **not** two different clocks.)

2. Vessel **leaves** the origin: **`LeftDockActual = t₁`**.

3. Vessel **arrives** at destination; we **complete** trip *N* and **start** trip *N+1* on the **same** tick **`t₂`**: **`ArrivedNextActual = t₂`**, **`EndTime = t₂`**.

4. **Trip *N+1*** on its **first** tick: **`StartTime = t₂`**, **`ArrivedCurrActual = t₂`** — matching **`ArrivedNextActual`** from trip *N* (chain invariant).
...
**Design rule:** On the canonical **arrival at destination dock** boundary, we **complete the current trip and start the next trip on the same tick**. Therefore there is **no** intentional split between “arrival stamped on tick T₁” and “completion on tick T₂” in the happy path: **one** `TimeStamp` applies to **`ArrivedNextActual`**, **`EndTime`** (and any legacy completion field being replaced), and the **next** trip’s **`ArrivedCurrActual`** / **`StartTime`** as defined for that transition.
...
|---------------------|------------------|--------|
| **`TripStart`**     | Split | **Coverage / chain:** in the happy path, **often equals `StartTime`** when the row opens (see **Worked example**); **not** automatically **`ArrivedCurrActual`**. **Physical:** only treat as origin arrival when the pipeline asserts it — otherwise **do not** use `TripStart` for ML/projection boundary facts. |
| **`AtDockActual`**  | Mostly **`StartTime`** / recording | **Not** a trusted **`ArrivedCurrActual`**; today it mixes first-seen, carry-forward, and fallback. Prefer **coverage** semantics or drop as a physical dock field after refactor. |
| **`TripEnd`**       | **`EndTime`** + optional **`ArrivedNextActual`** | **Coverage close:** **`EndTime`**. **Physical destination arrival:** **`ArrivedNextActual`** when asserted; same tick as **`EndTime`** in happy path. |
| **`ArriveDest`**    | **`ArrivedNextActual`** | Same role when trusted. |
| **`LeftDock` / `LeftDockActual`** | **`LeftDockActual`** (physical departure) + optional raw **`LeftDock`** | **Physical leave origin** for **`dep-dock`** projection and ML. |
