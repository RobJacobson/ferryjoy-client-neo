/**
 * Official crossing time configuration for scheduled trip arrival estimates.
 *
 * Client-safe copy of `convex/functions/scheduledTrips/sync/officialCrossingTimes.ts`.
 * We keep this in `src/` to avoid importing Convex/server modules into the app.
 *
 * Notes:
 * - Keys use the terminal pair format "FROM->TO".
 ...
 */
const formatTerminalPairKey = (departing: string, arriving: string): string =>
  `${departing}->${arriving}`;
