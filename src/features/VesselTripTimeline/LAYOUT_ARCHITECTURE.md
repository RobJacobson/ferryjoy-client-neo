...
- `trip.ArriveDest` or `trip.TripEnd` moves the active row to the final dock
  row
- departure evidence (`trip.LeftDock` or `vesselLocation.LeftDock`) moves the
  active row to the sea row
