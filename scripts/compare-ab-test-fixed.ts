...
const generateComparisonTable = (
  ...
  console.log(
    `  Average MAE difference: ${validDepartureMae.reduce((a, b) => a + b, 0) / validDepartureMae.length > 0 ? "+" : ""}${(validDepartureMae.reduce((a, b) => a + b, 0) / validDepartureMae.length).toFixed(3)}`
  );
  console.log(
    `  Average R² difference: ${validDepartureR2.reduce((a, b) => a + b, 0) / validDepartureR2.length > 0 ? "+" : ""}${(validDepartureR2.reduce((a, b) => a + b, 0) / validDepartureR2.length).toFixed(3)}`
  );
  console.log(
    `  Better MAE performance (${fileBLabel}): ${validDepartureMae.filter((d) => d < 0).length}/${validDepartureMae.length} terminal pairs`
  );
  console.log(
    `  Better R² performance (${fileBLabel}): ${validDepartureR2.filter((d) => d > 0).length}/${validDepartureR2.length} terminal pairs`
  );

  console.log("\nArrival Model:");
  console.log(
    `  Average MAE difference: ${validArrivalMae.reduce((a, b) => a + b, 0) / validArrivalMae.length > 0 ? "+" : ""}${(validArrivalMae.reduce((a, b) => a + b, 0) / validArrivalMae.length).toFixed(3)}`
  );
  console.log(
    `  Average R² difference: ${validArrivalR2.reduce((a, b) => a + b, 0) / validArrivalR2.length > 0 ? "+" : ""}${(validArrivalR2.reduce((a, b) => a + b, 0) / validArrivalR2.length).toFixed(3)}`
  );
  console.log(
    `  Better MAE performance (${fileBLabel}): ${validArrivalMae.filter((d) => d < 0).length}/${validArrivalMae.length} terminal pairs`
  );
  console.log(
    `  Better R² performance (${fileBLabel}): ${validArrivalR2.filter((d) => d > 0).length}/${validArrivalR2.length} terminal pairs`
  );
  ...
  console.log("\n=== COMPARISON SUMMARY ===\n");
  console.log(
    `Out of ${totalRoutes} routes tested with sufficient data (≥100 records):`
  );
  console.log(
    `- ${betterRoutes} route-model combinations perform better with ${fileBLabel}`
  );
  console.log(
    `- ${totalRoutes * 2 - betterRoutes} route-model combinations perform better with ${fileALabel}`
  );
...
const main = async () => {
  ...
  if (args.length !== 2) {
    console.error(
      "Usage: npm run compare:training-results <fileA.csv> <fileB.csv>"
    );
    console.error(
      "Example: npm run compare:training-results training-results-with-feature.csv training-results-without-feature.csv"
    );
    process.exit(1);
  }
