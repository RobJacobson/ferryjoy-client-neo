...
function generateComparisonTable(comparison: any[], fileALabel: string, fileBLabel: string): string {
...
`;
  ...
  if (comparison.length > 0) {
    ...
    const avgDepartureMaeA = comparison.reduce((sum, row) => sum + (row.departure_mae_file_a || 0), 0) / comparison.length;
    const avgDepartureMaeB = comparison.reduce((sum, row) => sum + (row.departure_mae_file_b || 0), 0) / comparison.length;
    const avgDepartureMaeDiff = comparison.reduce((sum, row) => sum + (row.departure_mae_diff || 0), 0) / comparison.length;

    const avgDepartureR2A = comparison.reduce((sum, row) => sum + (row.departure_r2_file_a || 0), 0) / comparison.length;
    const avgDepartureR2B = comparison.reduce((sum, row) => sum + (row.departure_r2_file_b || 0), 0) / comparison.length;
    const avgDepartureR2Diff = comparison.reduce((sum, row) => sum + (row.departure_r2_diff || 0), 0) / comparison.length;

    const avgArrivalMaeA = comparison.reduce((sum, row) => sum + (row.arrival_mae_file_a || 0), 0) / comparison.length;
    const avgArrivalMaeB = comparison.reduce((sum, row) => sum + (row.arrival_mae_file_b || 0), 0) / comparison.length;
    const avgArrivalMaeDiff = comparison.reduce((sum, row) => sum + (row.arrival_mae_diff || 0), 0) / comparison.length;

    const avgArrivalR2A = comparison.reduce((sum, row) => sum + (row.arrival_r2_file_a || 0), 0) / comparison.length;
    const avgArrivalR2B = comparison.reduce((sum, row) => sum + (row.arrival_r2_file_b || 0), 0) / comparison.length;
    const avgArrivalR2Diff = comparison.reduce((sum, row) => sum + (row.arrival_r2_diff || 0), 0) / comparison.length;

    const totalRecords = comparison.reduce((sum, row) => sum + row.total_records, 0);
...
async function compareSummaries() {
  ...
  if (args.length !== 2) {
    ...
    console.error('Example: npm run train:compare training-results-with-feature.csv training-results-without-feature.csv');
    console.error('Output: training-results-comparison.md');
    process.exit(1);
  }
  ...
  const markdownContent = generateComparisonTable(comparison, fileALabel, fileBLabel);

  // Write to file
  const outputPath = "training-results-comparison.md";
  writeFileSync(outputPath, markdownContent);

  console.log(`\nComparison complete! Results saved to ${outputPath}`);
  console.log(`Compared ${fileALabel} vs ${fileBLabel} across ${comparison.length} terminal pairs`);
}
