...
async function exportTrainingResults() {
  ...
  models.forEach((model) => {
    ...
    if (model.modelType === "departure") {
      pairMap.get(pairKey).departure_mae = metrics?.mae;
      pairMap.get(pairKey).departure_r2 = metrics?.r2;
      pairMap.get(pairKey).departure_rmse = metrics?.rmse;
      pairMap.get(pairKey).departure_std_dev = metrics?.stdDev;
    } else if (model.modelType === "arrival") {
      pairMap.get(pairKey).arrival_mae = metrics?.mae;
      pairMap.get(pairKey).arrival_r2 = metrics?.r2;
      pairMap.get(pairKey).arrival_rmse = metrics?.rmse;
      pairMap.get(pairKey).arrival_std_dev = metrics?.stdDev;
    }
  });
  ...
  // Write to file
  const outputPath = path.join(process.cwd(), "training-results.csv");
